...
exports.listAdmitted = async (req, res) => {
  try {
    const admissions = await adultOutpatientWardService.listAdmittedPatients(req.user.facility_id);
    const payload = admissions.map((row) => wardController.formatAdmissionForStaff(row));
    return success(res, payload);
  } catch (err) {
    return handleServiceError(res, err, 'Failed to load adult outpatient patients');
  }
};

exports.listDailyRecords = async (req, res) => {
  try {
    const records = await adultOutpatientWardService.listDailyRecords(
...
exports.saveDailyRecord = async (req, res) => {
  try {
    const record = await adultOutpatientWardService.saveDailyRecord({
      admissionId: req.params.admissionId,
      facilityId: req.user.facility_id,
      userId: req.user.id,
      body: req.body,
    });
    return success(res, { record }, 'Adult outpatient daily record saved');
  } catch (err) {
...
exports.transferToWard = async (req, res) => {
  try {
    const result = await adultOutpatientWardService.transferToWard({
      admissionId: req.params.admissionId,
      facilityId: req.user.facility_id,
      userId: req.user.id,
...
exports.dischargePatient = async (req, res) => {
  try {
    const result = await adultOutpatientWardService.dischargePatient({
      admissionId: req.params.admissionId,
      facilityId: req.user.facility_id,
      userId: req.user.id,
...
exports.transferToMortuary = async (req, res) => {
  try {
    const result = await adultOutpatientWardService.transferToMortuary({
      admissionId: req.params.admissionId,
      facilityId: req.user.facility_id,
      userId: req.user.id,
