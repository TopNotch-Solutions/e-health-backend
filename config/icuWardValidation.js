...
function validateIcuDailyRecord(body = {}) {
  const errors = [];
...
function validateIcuPorterTransport(transport = {}) {
  const errors = [];
  const add = (field, message) => errors.push({ field, message });
...
function validateIcuMortuaryTransfer({ cause_of_death, transport } = {}) {
  const errors = [];
...
module.exports = {
  ...
  REQUIRED_TRANSPORT_CHECKLIST_IDS,
};
