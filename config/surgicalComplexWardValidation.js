...
const TRANSFER_WARD_TYPES = ['general', 'icu', 'specialized_inpatient', 'outpatient_specialist'];
...
function validateSurgicalComplexPorterTransport(transport = {}) {
  const errors = [];
  const add = (field, message) => errors.push({ field, message });
...
function validateSurgicalComplexMortuaryTransfer({ cause_of_death, transport } = {}) {
  const errors = [];
  ...
  try {
    validateSurgicalComplexPorterTransport(transport);
  } catch (err) {
...
function validateSurgicalComplexWardTransfer({ target_ward_type, transport } = {}) {
  const errors = [];
