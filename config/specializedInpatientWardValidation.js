...
const TRANSFER_WARD_TYPES = ['general', 'icu', 'surgical_complex', 'outpatient_specialist'];
...
function validateSpecializedInpatientDailyRecord(body = {}) {
  const errors = [];
...
function validateSpecializedInpatientPorterTransport(transport = {}) {
  const errors = [];
  const add = (field, message) => errors.push({ field, message });
...
function validateSpecializedInpatientMortuaryTransfer({ cause_of_death, transport } = {}) {
  const errors = [];
  ...
  try {
    validateSpecializedInpatientPorterTransport(transport);
  } catch (err) {
...
function validateSpecializedInpatientWardTransfer({ target_ward_type, transport } = {}) {
  const errors = [];
...
function validateSpecializedInpatientDischarge({ discharge_notes } = {}) {
