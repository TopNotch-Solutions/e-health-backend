...
} = require('../config/adultOutpatientWardValidation');
const { onIcuTransferTransportCompleted } = require('./icuWardService');

const WARD_TYPE = 'adult_outpatient';

const WARD_TYPE_LABELS = {
  general: 'General ward',
  icu: 'ICU',
  specialized_inpatient: 'Inpatient',
};
...
async function loadAdultOutpatientAdmission(admissionId, facilityId, { transaction, lock } = {}) {
  ...
  if (admission.bed.ward.ward_type !== WARD_TYPE) {
    throw httpError('This action is only available for adult outpatient admissions', 400);
  }
  if (admission.status !== 'admitted') {
    throw httpError('Patient must be actively admitted in adult outpatient', 400);
  }
...
async function assertTodayDailyRecordSaved(admissionId, transaction) {
  const record = await AdultOutpatientDailyRecord.findOne({
    where: { admission_id: admissionId, record_date: todayDateString() },
    transaction,
  });
  if (!record) {
    throw httpError("Save today's adult outpatient daily record before this action", 400);
  }
}
...
async function listAdmittedPatients(facilityId) {
  ...
  return admissions.filter(
    (admission) => !admission.adultOutpatientDailyRecords?.length && !transferPending.has(admission.id)
  );
}

async function listDailyRecords(admissionId, facilityId) {
  await loadAdultOutpatientAdmission(admissionId, facilityId);
  const records = await AdultOutpatientDailyRecord.findAll({
...
async function upsertDailyRecordForAdmission({ admission, userId, body, transaction }) {
  validateAdultOutpatientDailyRecord(body);
  const recordDate = normalizeRecordDate(body.record_date);
  const vitals = pickVitals(body);

  const [record, created] = await AdultOutpatientDailyRecord.findOrCreate({
...
async function saveDailyRecord({ admissionId, facilityId, userId, body }) {
  ...
  try {
    const admission = await loadAdultOutpatientAdmission(admissionId, facilityId, { transaction: t, lock: true });
    ...
    notificationService.emitWardStaffQueueRefresh({ reason: 'adult_outpatient_daily_saved' });

    const refreshed = await AdultOutpatientDailyRecord.findByPk(record.id, {
      include: [{ association: 'recordedBy', attributes: ['id', 'first_name', 'last_name'] }],
    });
...
async function transferToWard({ admissionId, facilityId, userId, targetWardType, bedId, transport }) {
  validateAdultOutpatientWardTransfer({ target_ward_type: targetWardType, transport });
  const transportFields = resolveTransportFields(
    transport,
    `Adult outpatient to ${WARD_TYPE_LABELS[targetWardType] || targetWardType} transfer`
  );
  ...
  try {
    ...
    notificationService.emitWardStaffQueueRefresh({ reason: 'adult_outpatient_transfer' });
...
async function dischargePatient({ admissionId, facilityId, userId, discharge_notes }) {
  validateAdultOutpatientDischarge({ discharge_notes });
  ...
  try {
    ...
    const notes = discharge_notes?.trim() || 'Discharged from adult outpatient';
    ...
    notificationService.emitWardStaffQueueRefresh({ reason: 'adult_outpatient_discharge' });
...
async function transferToMortuary({
  ...
  validateAdultOutpatientMortuaryTransfer({ cause_of_death, transport });
  const mortuaryNote = notes?.trim() || cause_of_death?.trim() || null;
  const transportFields = resolveTransportFields(transport, mortuaryNote || 'Adult outpatient to mortuary');
